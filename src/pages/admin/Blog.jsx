...
export default function AdminBlog() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["posts", { page, search, status: statusFilter }],
    queryFn: () =>
      blogAPI.getAll({
        page,
        search,
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: ({ id, is_published }) =>
      blogAPI.update(id, { is_published: !is_published }),
    onSuccess: () => {
      queryClient.invalidateQueries(["posts"]);
  ...
  const handleToggleStatus = (id, currentStatus) => {
    toggleStatusMutation.mutate({ id, is_published: currentStatus });
  };

  const posts = data?.data || [];
  const pagination = data?.pagination || {};

  // Ensure all posts have tags as an array to prevent .some error
                ...
                {safePosts.map((post) => (
                  <tr key={post.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() =>
                          handleToggleStatus(post.id, post.is_published)
                        }
